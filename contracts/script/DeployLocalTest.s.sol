...
contract DeployLocal is Script, DeployPermit2 {
    ...
    function run() public {
        ...
        // ----------------------------- //
        vm.broadcast();
        SwapXHook swapxHook = new SwapXHook{salt: salt}(manager, taskManager);
        require(
            address(swapxHook) == hookAddress,
            "hook address mismatch"
        );
        console.log("Hook address: %s", address(swapxHook));
        ...
        vm.startBroadcast();
        testLifecycle(manager, address(swapxHook), posm, lpRouter, swapRouter);
        vm.stopBroadcast();
    }
