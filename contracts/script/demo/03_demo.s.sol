...
contract Demo03 is Script {
    ...
    // Case: SellOrderFulfilledByMultipleBuyOrders
    // Number of matchs: 2
    function run() public {
        ...
        swapRouter.swap(
            key,
            IPoolManager.SwapParams({zeroForOne: true, amountSpecified: -20, sqrtPriceLimitX96: 50}),
            testSettings,
        ...
        swapRouter.swap(
            key,
            IPoolManager.SwapParams({zeroForOne: true, amountSpecified: -30, sqrtPriceLimitX96: 30}),
            testSettings,
        ...
        swapRouter.swap(
            key,
            IPoolManager.SwapParams({zeroForOne: false, amountSpecified: -50, sqrtPriceLimitX96: 30}),
            testSettings,
            abi.encode(30, msg.sender)
        );
        vm.stopBroadcast();
